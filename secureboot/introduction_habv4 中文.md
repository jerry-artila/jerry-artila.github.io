...
## 1. 簡介
i.MX 系列處理器在片上 ROM 中提供高保證啟動(HAB)功能。ROM 負責從啟動媒體載入初始程式映像(U-Boot),而 HAB 使 ROM 能夠通過使用密碼學操作來驗證和/或解密程式映像。
...
## 1.1 HABv4 安全啟動架構
HABv4 安全啟動功能使用數字簽名來防止在設備啟動序列期間未經授權的軟體執行。如果惡意軟體控制了啟動序列,敏感數據、服務和網絡可能會受到影響。
HAB 認證基於使用 RSA 算法的公鑰密碼學,其中映像數據使用一系列私鑰離線簽名。然後使用相應的公鑰在 i.MX 處理器上驗證生成的已簽名映像數據。公鑰包含在 CSF 二進制文件中,而 SRK Hash 則編程在 SoC 保險絲中以建立信任根。
...
要編程到啟動媒體中的 U-Boot 映像需要正確構建,即它必須包含適當的命令序列文件(CSF)。
CSF 是由 HAB 解釋的二進制數據結構,用於指導認證過程,這是由代碼簽名工具[1]生成的。CSF 結構包含命令、SRK 表、簽名和證書。

關於安全啟動和代碼簽名工具(CST)的詳細信息可以在應用說明 AN4581[2]和安全啟動指南中找到。CSF 的語法和詳細信息可以在 CST 用戶指南中找到,該指南與 CST 工具一起打包並位於 doc 目錄中。
